...
new class extends Component
{
    ...
    public function mount(Approach $approach): void
    {
        $this->approach = $approach->loadCount('likes');

        $this->likesCount = $approach->likes_count;

        if (Auth::check()) {
            $like = Like::where('approach_id', $approach->id)
    ...
    public function toggleLike(): void
    {
        ...
        if ($this->userLikedId) {
            Like::where('id', $this->userLikedId)->delete();
            $this->userLikedId = null;
            $this->likesCount--;
        } else {
            $like = Like::create([
                'approach_id' => $this->approach->id,
                'user_id' => Auth::id(),
            ]);
            $this->userLikedId = $like->id;
            $this->likesCount++;
