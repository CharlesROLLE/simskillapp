<x-layouts::app :title="__('Edit VR Tool')">
    ...
    <form method="POST" action="{{ route('admin.vrtools.update', $vrtool) }}" enctype="multipart/form-data" class="mt-8 max-w-2xl space-y-6">
        @csrf
        @method('PUT')
        ...
        <flux:field>
            ...
            <div id="current-image" class="mb-3 {{ $vrtool->image ? '' : 'hidden' }}">
                <img src="{{ $vrtool->image ?? '' }}" alt="{{ $vrtool->title }}" class="w-48 h-32 object-cover rounded-lg border border-zinc-200 dark:border-zinc-700">
            </div>
